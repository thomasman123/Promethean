-- Fix GHL user role logic to not use "user" as default
-- GHL users should always be setter, sales_rep, or both - never just "user"

-- Update the role determination function
...
BEGIN
    ...
    -- (GHL users are added because they appear in sales/setter context)
    ...
    SET 
        ...
        roles = v_new_roles,
        -- Update primary_role to the role with most activity, or default to setter
        primary_role = CASE 
            ...
            ELSE 'setter'  -- Default to setter instead of 'user'
        END,
        updated_at = NOW()
...
-- Update the upsert function to default to 'setter' instead of 'user'
CREATE OR REPLACE FUNCTION public.upsert_ghl_user(
    ...
    p_primary_role TEXT DEFAULT 'setter'  -- Changed from 'user' to 'setter'
) RETURNS UUID AS $$
DECLARE
    v_user_id UUID;
BEGIN
...
-- Update any existing users with 'user' role to 'setter'
UPDATE public.ghl_users 
SET primary_role = 'setter', updated_at = NOW()
WHERE primary_role = 'user';

-- Update roles array for any users that might have 'user' in their roles
UPDATE public.ghl_users 
SET 
    roles = array_replace(roles, 'user', 'setter'),
    updated_at = NOW()
WHERE 'user' = ANY(roles);

-- Add comment explaining why we don't use 'user' role
COMMENT ON COLUMN public.ghl_users.primary_role IS 'Primary role in GHL system. Always setter, sales_rep, admin, or moderator - never generic user since GHL users are active in sales processes'; 