...
export function ViewsDropdown({ className = '', onCreateView }: ViewsDropdownProps) {
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <>
          {/* Backdrop for modal-like feel */}
          <div className="fixed inset-0 z-40" onClick={() => setIsOpen(false)} />
          
                    {/* Dropdown menu */}
          <div className="absolute top-full right-0 mt-2 min-w-[280px] bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-zinc-200/50 dark:border-zinc-800/50 z-50 max-h-96 overflow-hidden flex flex-col">
            {/* Scrollable content */}
            ...
            className={`w-[calc(100%-16px)] px-4 py-2.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all rounded-lg mx-2 ${
              !currentView ? 'bg-zinc-200/70 dark:bg-zinc-800/70' : ''
            }`}
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium text-zinc-900 dark:text-white">Default View</span>
              {!currentView && (
          ...
          {personalViews.length > 0 && (
            <>
              <div className="px-4 py-2 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider border-t border-zinc-200/50 dark:border-zinc-800/50 mt-2 pt-2">
                Personal Views
              </div>
              {personalViews.map((view) => (
                <button
                  ...
                  className={`w-full px-4 py-2.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all rounded-lg mx-2 ${
                ...
                >
                  <div className="flex items-center gap-3">
                    <svg className="w-4 h-4 text-zinc-500 dark:text-zinc-400" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/>
                    </svg>
                    <div className="flex-1">
                      <div className="font-medium text-zinc-900 dark:text-white">{view.name}</div>
                      {view.description && (
          ...
          {teamViews.length > 0 && (
            <>
              ...
              {/* Create View Button */}
              <div className="border-t border-zinc-200/50 dark:border-zinc-800/50 mt-2 pt-2">
                <button
                  onClick={handleCreateView}
                  className="w-full px-4 py-2.5 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all rounded-lg flex items-center gap-3"
                                  >
                    <svg className="w-4 h-4 text-blue-600 dark:text-blue-400" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/>
                    </svg>
                    <span className="text-sm font-medium text-zinc-900 dark:text-white">Create View</span>
                </button>
              </div>
            </div>
          </div>
